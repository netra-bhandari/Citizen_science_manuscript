...
path <- "F:/leipzig/CS spatial pattern/survey_data/Survey_data/figures in english"
...
#install.packages("tm")
library(tm)
library(gggibbous) #used for moonplots
library(cowplot)
library(patchwork)
library(tidyverse)
library(ggtextures)
...
completed <- survey_data %>% filter(survey_data$Letzte_Seit == 13)
...
(lik_imp_obs <- plot(likert(imp_obs), center=2, plot.percents=F, plot.percent.low=T,
                    plot.percent.high=T,legend.position = "right",text.size=14, ordered=FALSE,
...
groups <- tibble(
  ...
  image = c("./symbols/amphibians.png","./symbols/bee.png","./symbols/beetle.png",
            "./symbols/dragonfly.png","./symbols/plant.png","./symbols/butterfly.png","./symbols/reptile.png","./symbols/bird.png"))
...
(taxa_2 <- ggplot(new_groups, aes( y= perc, x= sort(group), 
...
#write.csv(groups_others,"groups_others.csv")#change in deepl to english names

english_names <- read.csv("groups_others.csv") #use from dropbox

groups_others$eng_group <- english_names$Group.1

colnames(groups_others) <- c("de_group","Frequency","en_group")


#create a treemap=============================================================================
#png("Please select ONE species group.png")
...
#write.csv( prior_data,"prior_de.csv")
prior_data <- read.csv("prior_de.csv") #this contains translated columns (check dropbox)

#create tree map by transforming variables into fators==============================================
prior_data <- transform(prior_data, data.available = factor(!is.na(subgroup)), x = 1) 
...
saveWidget(inter, file=paste0( getwd(), "/interactiveTreemap.html"),selfcontained=TRUE,
           knitrOptions=list())
...
#write.csv(all_questions, "all_questions.csv")
...
ques3_sel_hi_lo$Question <- str_replace_all(ques3_sel_hi_lo$Question, 
                                            c(" das Resultat einer aktiven Suche .Sie sind zum Beispiel an einen bestimmten Ort 
                                              gefahren. um gezielt nach Arten zu suchen. ." = "the result of an active search (for 
...
ques4a$species_group <- as.factor(ques4a$species_group)
...
ques6$species_group <- factor(ques6$species_group)
...
ques7$Question <- gsub(".Falle_verwendet_haben.wie_lang_war_typischerweise_das_Erfassungs.Zeitfenster.__."," ",ques7$Question)
...
text <- ques7$Response
text <- read.csv("types_of_traps.csv") #from dropbox
...
#png("Which_types_of_XXX_traps_do_you_use.png", width=12, height=8, units="in", res=300)
...
ques9$Question <- str_replace_all(ques9$Question, 
                                  c("Ich vermute um welche Art es sich handelt" = "I suspect which species it is",                                                        "Ich melde die Art nicht"= 
                                    ...
                                    "Ich melde die Beobachtung auf der taxonomischen Ebene die ich sicher identifizieren kann z B 
...
(ques9_plot_new <- ggplot(ques9_hi_lo, aes(fill= response, y= perc, x=Question)) + 
                    geom_bar( position="stack", stat="identity", width=0.5, alpha = 0.6) +
                    scale_fill_brewer(palette = "BrBG", direction =-1) +
                    coord_flip()+
                    theme_classic() +
                    xlab("")+ 
...
share_obs <- read.csv("share_obs_en.csv", header = T) #from dropbox
...
#png("How_do_you_otherwise_share_your_observations_with_others.png", width=12, height=8, units="in", res=300)
...
(lik1 <- plot(likert(areas), center=2, plot.percents=F,
              ...
              ordered=FALSE, colors = c("#8c510a","#d8b365","#f6e8c3","#c7eae5","#5ab4ac","#01665e"))+
              theme(strip.text=element_text(size=50),
                    axis.text.y = element_text(size = 50, color = "black"),
                    axis.text.x = element_text(size = 50, color = "black"), 
                    axis.title.x = element_text(size = 50, color = "black"),   
...
DEU <- readRDS("gadm36_DEU_1_sp.rds")
...
postcode_shp <- st_read("postalCodesGermany1st2letters.shp")
...
comments <- read.csv("comments_en.csv", header =  T) #english version
...
set.seed(1234) # for reproducibility 
png("Do_you_have_any_other_comments_Here_you_can_make_additional_comments_on_the_questions_or_the_survey.png",width=12, height=8, units="in", res=300)
